...
function getArray(items: any[]):any {       // gli items sono di tipo any, e anche il mio array è di tipo any
...
let numberArray = getArray([5,10,15,20]);                       // posso passare numeri
let stringArray = getArray(["cats","dogs","ducks","birds"])     // posso passare stringhe
...
function getArray1 <T>(items: T[]): T[]{                        //è una prassi usare T ( modo comune per indicare il modello generico)
...
let numberArray1 = getArray1<number>([5,10,15,20]);
...
let numberArray2 = getArray1<string>(["cats","dogs","ducks","birds"]);
...
let numberArray3 = getArray1(["cats",15,]);
