...
function arrayRandom(){                             //creo la funzione arrayRandom che non richieda parametri in ingresso.
  ...
  let numberoRandom = 0;                            //creo la variabile numero random nella quale verrà memorizzato un numero random ( più avanti nel codice)
  let i = 0;                                        // creo l'indice i utile per il ciclo while ( passaggio successivo )

  while(i < 10){                                    // cicla finché indice non sarà superiore a 10 ( in questo caso quindi, una volta chiuso il ciclo creerà un array di 10 elementi)
    numberoRandom = Math.floor(Math.random()*11);   // alla variabile numeroRandom affido un numero random da 1 - 10 incluso
    if(!array.includes(numberoRandom)){             // avvio un if dove dirò se in tutto l'array non esiste il valore presente ( numeroRandom ) allora verifica la condizione interna

      array.push(numberoRandom);                    // se la condizione sarà verificata nell'if, pusho nel nostro array la variabile numeroRandom
      i++;                                          // aumento il contatore del while ( nel nostro caso, solo se la condizione dell'if sarà verificata)
    }
  }
