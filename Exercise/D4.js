...
for (let i = 0; i < starWarsCharacters.length; i++) { // ciclo per x volte quanto è la lunghezza di dell'array da cui prelevare

    let name = starWarsCharacters[i].name; // dichiaro i
    characters.push(name)  //pusho dentro characters tutti gli elementi nome presenti in ogni oggetto dentro SWC.
...
let femaleCharacters = [];  // creo un nuovo array

let x = {                   // creo un nuovo oggetto
  name: 'Leia Organa',
  hair_color: 'brown',
  eye_color: 'brown'
}
...
for (let i = 0; i < starWarsCharacters.length; i++) { // ciclo

let t = starWarsCharacters[i]; // i = occhi

switch (t.eye_color) {                        // switch
    case 'blue':                    // cond per ogni variabile
    eyeColor.blue.push(t)
    break;
    case 'yellow': 
    eyeColor.yellow.push(t)
    break;
    case 'brown': 
    eyeColor.brown.push(t)
    break;
    case 'red': 
    eyeColor.red.push(t)
    break;
    case 'blue_gray': 
    eyeColor.blue_gray.push(t)
    break;
  default:
    break;
}
...
console.log('La massa totale è di: ' + totalMass);
}
